...
class Rocket(Flight):
    def __init__(self, ho2_per=35, others_mass=2, tank_per=0.5, KPD=0.8, mdot=0.46, ch_pressure=3):
        self.gap_width = 5 * (10**(-3))
        # ratio of oxidizer to fuel, mass flow rate of the propellant, total height of ox+fuel tanks.
        self.mdot, self.radius, self.Cd, self.tanks_height, self.others = mdot, 0.06, 0.7, 1, others_mass
        self.ho2_dens = 1_450_000/((ho2_per*-4.50)+1450)
        self.ho2_per = ho2_per  # perentage of h2o2 in oxidizer
        self.o_f = self.ho2_dens / 800
        self.engine = Engine(ch_pressure=ch_pressure, mdot=mdot, KPD=KPD, o_f=self.o_f, ho2_per=ho2_per)
        self.tanks = Tanks(o_f=self.o_f, mdot=mdot, Po=8, tank_per=tank_per)  # oxidizer, gas, fuel tanks
    ...
    def plot_rocket(self, ix=999, save=False):
        ...
        plt.figtext(0.55, 0.89, s=f"Po={round(self.tanks.Po, 2)}", fontsize='12')
    ...
    def __repr__(self):
        return f"Rocket thrust: {rnd(self.engine.thrust)} Newtons" "\n" \
               ...
               f"Gas pressure {rnd(self.tanks.Po)} \ {rnd(self.tanks.Pf)} bar" "\n" \
